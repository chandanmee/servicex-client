...
export default function ResponsiveWrapper({ children, className = "", max = "lg", padded = true }: Props) {
  const maxClass =
    max === "sm" ? "max-w-screen-sm" : max === "md" ? "max-w-screen-md" : max === "lg" ? "max-w-screen-lg" : max === "xl" ? "max-w-screen-xl" : "max-w-screen-2xl";
  return <div className={`mx-auto ${maxClass} ${padded ? "px-4 md:px-6" : ""} ${className}`}>{children}</div>;
}
